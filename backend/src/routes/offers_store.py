...
def create():
    ...
    if not target_user:
        return jsonify({'message': 'target_user required'}), 400
    ...
    try:
        ...
        return jsonify({'message': 'invalid_identity'}), 400
    ...
    if waiting_state not in ('lobby', ''):
        return jsonify({'message': 'cannot_create_offer_in_current_status', 'status': waiting_state}), 409
...
def cancel():
    ...
    if not oid:
        return jsonify({'message': 'offer_id required'}), 400
    db = _db()
...
def accept():
    ...
    if not offer:
        return jsonify({'message': 'offer_not_found'}), 404
    ...
    players = {
        'sente': {'user_id': sente_uid, 'username': s_name, 'rating': 0},
        'gote':  {'user_id': gote_uid,  'username': g_name, 'rating': 0},
    }
