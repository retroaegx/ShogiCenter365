...
def _format_kif(
    ...
    # 参考: https://kakinoki.o.oo7.jp/kif_format.html
    if winner in ("sente", "gote") and last_ply_printed > 0:
...
def _summary_from_doc(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "players": {
            "sente": {"username": sente.get("username") or "", "user_id": _safe_id(sente.get("user_id") or "")},
            "gote": {"username": gote.get("username") or "", "user_id": _safe_id(gote.get("user_id") or "")},
        },
...
def search_kifu():
    ...
    if isinstance(result, str) and result in ("sente", "gote"):
        q["winner"] = result
    # draw is not implemented in core game_service, but keep API compatible
    elif isinstance(result, str) and result == "draw":
        q["finished_reason"] = {"$regex": "draw", "$options": "i"}
    ...
    try:
        # We deliberately avoid count_documents() here.
        # Counting on large collections (especially with OR filters) is a major
        # source of 15s client timeouts.
        cur = (
            games.find(q, proj)
        ...
        try:
            # Abort slow queries rather than hanging the whole request.
            cur = cur.max_time_ms(8000)
        except Exception:
            pass
        docs_plus = list(cur)
    except Exception as e:
        current_app.logger.exception("kifu.search failed: %s", e)
