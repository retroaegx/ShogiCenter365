...
def public_contact_send():
    ...
    if not to_email:
        return jsonify({'success': False, 'message': 'サーバーの問い合わせ受信先が未設定です'}), 500
    ...
    if not name or not subject or not email or not body:
        return jsonify({'success': False, 'message': 'name / subject / email / body は必須です'}), 400

    # Minimal validation to reduce abuse/header-injection
    if any(x in subject for x in ('\r', '\n')) or any(x in name for x in ('\r', '\n')):
        return jsonify({'success': False, 'message': '入力が不正です'}), 400
    if any(x in email for x in ('\r', '\n')):
        return jsonify({'success': False, 'message': '入力が不正です'}), 400
    if len(body) > 8000:
        return jsonify({'success': False, 'message': '本文が長すぎます'}), 400
    ...
    try:
        ...
        return jsonify({'success': False, 'message': '送信に失敗しました'}), 500

    return jsonify({'success': True, 'message': '送信しました'}), 200
