...
export const AuthProvider = ({ children }) => {
  ...
  // 新規登録
  const register = useCallback(async (username, email, password, rating) => {
  ...
  const login = useCallback(async (identifier, password) => {
    try {
      ...
      if (data) return data;
      return { success: false, message: (e && (e.message || e.toString())) || 'ログインに失敗しました' };
    }
  ...
  const loginAsGuest = useCallback(async (rating) => {
    const payload = {}
    if (rating != null && Number.isFinite(Number(rating))) payload.rating = Number(rating)
  ...
  const completeGoogleSignup = useCallback(async (signupToken, username, rating) => {
    const res = await api.post('/auth/google/complete', { signup_token: signupToken, username, rating });
    const t = res?.data?.access_token;
    if (t) await persistTokenAndLoadProfile(t);
    return res?.data;
  }, [persistTokenAndLoadProfile]);
