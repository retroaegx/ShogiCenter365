...
export default function IncomingOfferLayer() {
  ...
  return (
    <AlertDialog open={open} onOpenChange={(v) => { if (!v) reset(); }}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>対局申請が来ました</AlertDialogTitle>
          <AlertDialogDescription>
            <div className="space-y-1">
              <div>相手: <span className="font-semibold">{fromName || '—'}</span>{fromRating != null ? <span className="ml-2 text-gray-500">〔R {fromRating}〕</span> : null}</div>
              <div>形式: {gameType === 'rating' ? 'R対局' : '自由対局'}</div>
              <div>持ち時間: {timeLabel || '—'}</div>
            </div>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel disabled={busy} onClick={decline}>拒否</AlertDialogCancel>
          <AlertDialogAction disabled={busy} onClick={accept}>承諾</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  );
