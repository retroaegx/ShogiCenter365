...
export const PIECE_NAMES = {
  ...
  [PIECE_TYPES.ROOK]: '飛',
  [PIECE_TYPES.KING]: '王',
  [PIECE_TYPES.PROMOTED_PAWN]: 'と',
  [PIECE_TYPES.PROMOTED_LANCE]: '成香',
  [PIECE_TYPES.PROMOTED_KNIGHT]: '成桂',
  [PIECE_TYPES.PROMOTED_SILVER]: '成銀',
  [PIECE_TYPES.PROMOTED_BISHOP]: '馬',
  [PIECE_TYPES.PROMOTED_ROOK]: '竜'
};
...
export function makeMove(gameState, fromRow, fromCol, toRow, toCol, promote = false) {
  ...
  if (wouldBeInCheckAfterMove(board, fromRow, fromCol, toRow, toCol, currentPlayer)) {
    return { success: false, message: '王手を回避してください' };
  }
  ...
  if (promote) {
    if (!canPromote(piece, fromRow, toRow)) {
      return { success: false, message: 'その駒は成れません' };
    }
...
export function makeDrop(gameState, row, col, pieceType) {
  ...
  if (!playerCaptured[pieceType] || playerCaptured[pieceType] <= 0) {
    return { success: false, message: 'その駒を持っていません' };
  }
  ...
  if (!targetMove) {
    return { success: false, message: 'その位置には駒を打てません' };
  }
...
export function shogiToCoordinate(shogiNotation) {
  ...
  const file = shogiNotation[0];
  const rank = shogiNotation[1];
